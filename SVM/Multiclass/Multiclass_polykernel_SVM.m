function test_label = Multiclass_polykernel_SVM(training_data,training_label,test_data,beta,deg)
...
for epoch=1:2
    ...
    for p = perm
        ...
        A = polyn_kernel(data(p,:),data(p,:),deg);
        ones_y_p = zeros(k,1);
        ...
        for i=1:m
            if i~=p
                B = B + polyn_kernel(data(p,:),data(i,:),deg) * Tau(:,i);
            end
        end
        B = B - beta * ones_y_p;
...
for i=1:length(test_data(:,1))
    for r =1:k
        M(i,r) = Tau(r,:) * polyn_kernel_matrix(test_data(i,:),data,deg)';
    end
end
